...
public class KDTree<T> {
	...
	/*
	 * convert kd-tree to a metric index, add
	
	public double[] traverseConvert(indexNode rootKmeans, KDNode<T> scanNode, int dimension) {
		//if the capacity has not be achieved, keep traversing until
		rootKmeans.setPivot(scanNode.k.coord);//HRect can be used to.
		// construct the hrec
		rootKmeans.addSinglePoint((int)(scanNode.v));
		
		// the radius can be estimated based on the minimum distance and the size of the radius, without scanning every point
		if(scanNode.left == null ) {// if it is a leaf node
			rootKmeans.setSum(scanNode.k.coord);
			return scanNode.k.coord;
		}else {// if it is a 
			//get t, left and right.
			rootKmeans.addSum(scanNode.k.coord);//
		//	traverseConvert();
		//	rootKmeans.addSum(scanNode.left.);
		}
		return null;
		//get sum
		//get the radius, and 
