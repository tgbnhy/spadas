...
class KDNode<T> {
	...
	protected static <T> int edit(HPoint key, Editor<T> editor, KDNode<T> t,
			int lev, int K) throws KeyDuplicateException {
	...
	// Method srch translated from 352.srch.c of Gonnet & Baeza-Yates, K is the dimension
	...
	protected static <T> void rsearch(HPoint lowk, HPoint uppk, KDNode<T> t,
			int lev, int K, List<KDNode<T>> v) {
