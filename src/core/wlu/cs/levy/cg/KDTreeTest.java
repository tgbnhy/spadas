...
public class KDTreeTest {
	...
	public void testDelete() throws KDException {
		...
		try {
			kt.delete(targ);
			assertFalse(true);
		} catch (core.wlu.cs.levy.cg.KeyMissingException e) {
			// supposed to be here
		}
		kt.delete(targ, true);
	...
	public void testEditing() throws KDException {
		...
		try {
			kt.insert(targ, p2);
			assertFalse(true);
		} catch (core.wlu.cs.levy.cg.KeyDuplicateException e) {
			// supposed to be here
		}
	...
	public void testNearestNeighborWithSampleData() throws KDException {
		...
		assertArrayEquals(new Long[] { 6l, 1l }, nbrs.toArray(new Long[nbrs.size()]));
