...
public class Search {
	...
	static void hausdorffEarylyAbandonging(Map<Integer, double[][]> dataMap, int querySet, Map<Integer, indexNode> indexMap, 
		...
		for(int a=1; a<=dataMap.size(); a++) {// access the dataset by files or in memory directly, find the datasetMapping to read
			...
			if(topkEarlyBreaking)//terminate when index is not
