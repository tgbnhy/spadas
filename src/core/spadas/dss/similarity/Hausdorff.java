...
public class Hausdorff {
	...
	/*
	 * set according to the dimension except the longitude and latitude
	 */
	static void setWeight(int dimension) {
	...
	/*
	 * set according to the weight file
	 */
	static void setWeight(int dimension, String filename) {
	 ...
	 */
	public static double EuclideanDis(double []point1xy, double []point2xy, int dimension) {
		if (weight == null) {
			double sum = 0;
			for (int i = 0; i < dimension; i++) {
			...
			double sum = 0;
			for (int i = 0; i < dimension; i++) {
				sum += Math.pow((point1xy[i] - point2xy[i])*weight[i], 2);
			}
			return Math.sqrt(sum);
		}
	}
	...
	/*
	 * computed the distance one selected dimensions
	 */
	static double EuclideanDis(double []point1xy, double []point2xy, int dimension, boolean nonSelectedDim[]) {
		if (weight == null) {
			double sum = 0;
			for (int i = 0; i < dimension; i++) {
				if (nonSelectedDim[i] == false)
					sum += Math.pow((point1xy[i] - point2xy[i]), 2);
			}
			return Math.sqrt(sum);
		}else {
...
//        return result;
	...
	public static double naiveHausdorff(double[][] a, double[][] b, int dimension) {
		double maxDist = 0;
		...
		for (double[] aa : a) {
			...
			for (double[] bb : b) {
				dist = EuclideanDis(aa, bb, dimension);
		...
		for (int i=0; i<length; i++) {
			...
			for (queueSecond aQueueSecond : secondHeaps) {
				...
				if (newnodeb != null) {
					...
					pivot_distance = EuclideanDis(point2xys[aQueueSecond.getPointId()-1], pivtoList.get(i), dimension);
				}
				...
				double newub = pivot_distance + bradius + radiusArrayList.get(i);// the loose bound here, we use a tighter one in the adavanced version
				if (newub < minub)
					minub = newub;
			}
			if(anode.isLeaf()) {
    ...
    static double traverseY(indexNode bnode, double[][] point1xys, double[][] point2xys, PriorityQueue<queueSecond> secondHeaps, 
		...
		for (int i=0; i<length; i++) {
			...
			if (anode != null) {
				...
				pivot_distance = EuclideanDis(point1xys[apoint-1], pivtoList.get(i), dimension);
			}
			if(bnode.isLeaf()) {
    ...
    /*
	 * next step in the Hausdorff evaluation, put outlier alone
	 * ther it can accelerate the performance, see the effect of outliers
	 * see whe
	 */
