...
package tree.trajectory.clustering.kpaths;
...
/*
	...
	/*
	 * we compute the distance on Euclidean space, and users can specify any dimensions
	 */
	public static double EuclideanDisAny(double []a, double []b, int dimension, Boolean dimeBoolean[], Boolean dimensionAll) {
		double distance = 0;
		for(int i = 0; i<dimension; i++ ) {
			if(dimensionAll || dimeBoolean[i]==false)
				distance += Math.pow(Math.abs(a[i]- b[i]), 2);// the l2-norm
		}
		distance = Math.sqrt(distance);
		return distance;
