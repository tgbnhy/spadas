...
public class CoverTree<E> {
	...
	public void traverseConvert(indexNode rootKmeans, int dimension){
		rootNode.traverseConvert(rootKmeans, dimension);
	}
	...
	class Node<E> implements Comparable<Node<E>> {
		...
		/*
		 * may be this can be changed to support the distance, 
		 * or we use the overlapped range to compute the distance rather than using the complex distance, but not sure whether they obeys triangle inequality
		 * we propose new distance between balls by their pivot distance and radius, what should be the right distance, whether it is metric, and used for pruning, 
		 */
		...
		/*
		 * convert two a tree format used by UniK, how to find the leaf node is crucial
		 */
		public int traverseConvert(indexNode rootKmeans, int dimension) {
			rootKmeans.setRadius(distance);
			rootKmeans.setPivot(point);//
			if(children.isEmpty()){//for the leaf node
				Set<Integer> aIntegers = new HashSet<Integer>();
			//	aIntegers.add((int)element+1);//this is not true in jdk1.8
				rootKmeans.setSum(point);
				rootKmeans.addPoint(aIntegers);
				rootKmeans.setTotalCoveredPoints(1);
				return 1;
			}else {
				int count = 0;
				for(Node<E> chp : children){
